...
function nearestDistrict(lat, lng) {
  return Object.entries(ANCHORS).reduce((closest, [name, point]) =>
    havKm({ lat, lng }, point) < closest.distance ? { name, distance: havKm({ lat, lng }, point) } : closest,
  { name: "Nagpur", distance: Infinity }).name;
}
...
async function liveWeatherFor(lat, lng) {
  ...
  return {
    ...
    leafWetness: Math.round(Math.min(100, data.current.relative_humidity_2m * 0.6 + rainProb * 0.4)),
...
// backend/app.py returns labels from the model trained on the supplied tomato dataset.
const MODEL_LABELS = {
  "Tomato Early blight leaf": { diseaseKey: "early_blight", type: "fungal" },
  "Tomato leaf late blight": { diseaseKey: "late_blight", type: "fungal" },
  "Tomato mold leaf": { diseaseKey: "generic", type: "fungal" },
  "Tomato leaf bacterial spot": { diseaseKey: "generic", type: "unknown" },
  "Tomato Septoria leaf spot": { diseaseKey: "generic", type: "fungal" },
  "Tomato leaf mosaic virus": { diseaseKey: "generic", type: "unknown" },
  "Tomato leaf yellow virus": { diseaseKey: "generic", type: "unknown" },
  "Tomato leaf": { diseaseKey: "generic", type: "unknown" },
};

async function runModelInference(file, cropKey) {
  const form = new FormData();
  form.append("image", file);
  const response = await fetch(`${import.meta.env.VITE_API_URL || "http://127.0.0.1:8000"}/predict`, { method: "POST", body: form });
  if (!response.ok) throw new Error((await response.json().catch(() => ({}))).detail || "Prediction service unavailable");
  const prediction = await response.json();
  if (!prediction.detected) throw new Error("No leaf symptoms detected in this image");
  const mapped = MODEL_LABELS[prediction.label] || { diseaseKey: "generic", type: "unknown" };
  const disease = DISEASES[mapped.diseaseKey];
  const severityPct = prediction.coveragePct ?? 0;
  return {
    crop: cropKey === "other" ? "tomato" : cropKey,
    cropConfidence: 100,
    diseaseKey: mapped.diseaseKey,
    label: prediction.label,
    type: mapped.type,
    confidence: prediction.confidence,
    severityPct,
    band: sevBand(severityPct).key,
    indicators: prediction.boxes.map((box) => `${box.label} (${box.confidence}%)`),
    causes: [pickL(disease.why, "en")],
    provider: "KrishiRakshak YOLO model",
    isDemo: false,
...
function FarmerApp({ lang, reports, setReports, alerts, setAlerts, online, lastScan, setLastScan, setToast }) {
  ...
  const [fieldContext, setFieldContext] = useState({
    location: { lat: ANCHORS.Nagpur.lat, lng: ANCHORS.Nagpur.lng, district: "Nagpur", isLive: false },
    weather: WEATHER, loading: false, attempted: false,
  });
  ...
  const refreshFieldContext = async () => {
    setFieldContext((current) => ({ ...current, loading: true }));
    try {
      const location = await deviceLocation();
      let weather = WEATHER;
      let weatherLive = false;
      try { weather = await liveWeatherFor(location.lat, location.lng); weatherLive = true; } catch { /* use explicit fallback weather */ }
      const next = {
        location: { ...location, district: "GPS location", isLive: true },
        weather, weatherLive, loading: false, attempted: true,
      };
      setFieldContext(next);
      return next;
    } catch {
      const fallback = {
        location: { lat: ANCHORS.Nagpur.lat, lng: ANCHORS.Nagpur.lng, district: "Nagpur", isLive: false },
        weather: WEATHER, weatherLive: false, loading: false, attempted: true,
      };
      setFieldContext(fallback);
      setToast(pickL(L("Location permission was unavailable — using demo location.", "स्थान अनुमति उपलब्ध नहीं थी — डेमो स्थान का उपयोग हो रहा है।", "स्थान परवानगी उपलब्ध नाही — डेमो स्थान वापरत आहे।"), lang));
      return fallback;
    }
  };

  useEffect(() => {
    if (view === "home" && !fieldContext.attempted && !fieldContext.loading) void refreshFieldContext();
    // Location is requested once per page load; a new scan refreshes it again.
    // eslint-disable-next-line
  }, [view, fieldContext.attempted, fieldContext.loading]);
  ...
  useEffect(() => {
    ...
    setStep(0);
    let i = 0;
    const iv = setInterval(() => { i += 1; setStep(i); if (i >= STEPS.length) { clearInterval(iv); void finish(); } }, 520);
    return () => clearInterval(iv);
    // eslint-disable-next-line
  }, [view]);

  const finish = async () => {
    const context = await refreshFieldContext();
    const near = crop ? reports.filter((r) => r.disease === PRIMARY[crop] && havKm(r, context.location) <= 12 && r.daysAgo <= 7).length : 0;
    let d;
    try {
      d = imgFile ? await runModelInference(imgFile, crop) : runDemoInference(crop);
    } catch (error) {
      setToast(pickL(L("Model is unavailable — showing the demo result.", "मॉडल उपलब्ध नहीं है — डेमो परिणाम दिखाया जा रहा है।", "मॉडेल उपलब्ध नाही — डेमो निकाल दाखवला आहे।"), lang));
      d = runDemoInference(crop);
    }
    const wr = weatherRisk(context.weather, d.type);
    ...
    setWOverride({ humidity: context.weather.humidity, rainProb: context.weather.rainProb });
  ...
  const liveWeather = { ...fieldContext.weather, humidity: wOverride.humidity, rainProb: wOverride.rainProb };
  ...
  const submitReport = () => {
    ...
    const nr = {
      id, crop, disease: dx.diseaseKey, district: fieldContext.location.district, lat: fieldContext.location.lat, lng: fieldContext.location.lng,
    ...
    const newAlerts = [{ id: "A" + id, audience: "officer", type: "new_report", read: false, createdAt: Date.now(),
      ...
      body: `${dx.label || dName(dx.diseaseKey)} · ${cName(crop)} · ${id}`, district: fieldContext.location.district }];
    if (highRisk) newAlerts.unshift({ id: "AH" + id, audience: "farmers", type: "high_risk", read: false, createdAt: Date.now(),
      ...
      body: `${dx.label || dName(dx.diseaseKey)} · ${cName(crop)} · ${pickL(riskName[nr.riskLevel] || riskName.high, lang)}`, district: fieldContext.location.district });
        ...
        <Card style={{ marginBottom: 12 }}>
          <div className="mb-2 flex items-center justify-between">
            <div className="text-xs font-medium" style={{ color: C.sub }}>
              {fieldContext.loading ? "Getting local weather…" : fieldContext.location.isLive ? `${t("weather")} · GPS` : `${t("weather")} · Nagpur (demo)`}
            </div>
            <CloudRain size={16} style={{ color: C.sky }} />
          </div>
          <div className="flex items-center justify-between">
            {[[Thermometer, fieldContext.weather.tempC + "°", t("temp")], [Droplets, fieldContext.weather.humidity + "%", t("humidity")], [CloudRain, fieldContext.weather.rainProb + "%", t("rain")], [Wind, fieldContext.weather.windKph, t("wind")]].map(([Ic, v, l], i) => (
    ...
    return (
      <Shell back={() => setView("capture")} title={t("retake")}>
        <div className="mb-2 flex items-center justify-between">
          ...
          <Badge color={C.amber} bg={C.amberSoft}>{dx.isDemo ? t("demoBadge") : "Trained AI model"}</Badge>
        </div>
        ...
        <div className="mb-3 rounded-xl px-3 py-2.5" style={{ background: tierColor + "12", border: `1px solid ${tierColor}33` }}>
          <div className="font-semibold" style={{ color: tierColor }}>
            {pickL(tier.verb, lang)} {dx.label || dName(dx.diseaseKey)}
          </div>
        ...
        <Card style={{ marginBottom: 12 }}>
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-lg font-bold" style={{ color: C.text }}>{dx.label || dName(dx.diseaseKey)}</div>
            </div>
  ...
  if (view === "advisory" && dx) {
    ...
    return (
      <Shell back={() => setView("result")} title={t("detected")}>
        <div className="mb-3">
          <div className="text-xl font-bold" style={{ color: C.text }}>{dx.label || dName(dx.diseaseKey)}</div>
  ...
  if (view === "compose" && dx) {
    return (
      <Shell back={() => setView("result")} title={t("detected")}>
        ...
        <div className="mb-3 flex items-center gap-2 rounded-lg px-3 py-2 text-sm" style={{ background: C.paper, color: C.sub }}>
          <MapPin size={15} style={{ color: fieldContext.location.isLive ? C.green : C.amber }} />
          {fieldContext.loading
            ? "Getting GPS and weather…"
            : fieldContext.location.isLive
              ? `GPS: ${fieldContext.location.lat.toFixed(4)}, ${fieldContext.location.lng.toFixed(4)} · ${fieldContext.weatherLive ? "live weather attached" : "weather fallback"}`
              : "Demo location · Nagpur, Maharashtra"}
        </div>
